...
function getLoginRedirectURL(redirectUrl, privacyCache, newlyRegisteredUser) {
    ...
    if (argsForQueryString) {
        ...
        result = URLUtils.url(endpoint, "registration", registered)
            .relative()
            .toString();
    }
...
function getPasswordResetToken(customer) {
    ...
    Transaction.wrap(function () {
        passwordResetToken =
            customer.profile.credentials.createResetPasswordToken();
    });
...
function sendPasswordResetEmail(email, resettingCustomer) {
    ...
    const externalResponse = externalDataServiceHelpers.setPasswordResetToken(
    ...
    if (externalResponse.ok) {
        emailHelpers.sendEmail(
            emailObj,
            "account/password/passwordResetEmail",
            objectForEmail
        );
    }
}
...
function sendAccountEditedEmail(profile) {
    ...
    emailHelpers.sendEmail(
        emailObj,
        "account/components/accountEditedEmail",
        userObject
    );
}
...
function loginCustomer(email, password, rememberMe) {
    ...
    return Transaction.wrap(function () {
        ...
        if (authenticateCustomerResult.status !== "AUTH_OK") {
            ...
            var errorMessageKey =
                errorCodes[authenticateCustomerResult.status] ||
                errorCodes.default;
